...
const mockReels = [
  {
    ...
    author: { name: 'neon_night', avatar: 'https://i.pravatar.cc/150?u=1', followed: false },
    description: 'Nights in the digital city. Experience the glow of futuristic architecture and vibrant lights. #cyberpunk #neon #vibes #future',
    music: 'Digital Dreams - Synthwave Artist',
    likes: '124.5k',
    comments: '1,240',
    shares: '4.2k'
  },
  {
    ...
    author: { name: 'nature_lover', avatar: 'https://i.pravatar.cc/150?u=2', followed: true },
    description: 'The golden hour magic. Witness the beauty of nature in its purest form. 🍂☀️ #autumn #nature #peace #meditation',
    music: 'Forest Whispers - Nature Soundscape',
    likes: '82k',
    comments: '456',
    shares: '1.5k'
  },
  {
    ...
    author: { name: 'future_mind', avatar: 'https://i.pravatar.cc/150?u=3', followed: false },
    description: 'Scaling your digital skills for the future. The evolution of online learning is here. #tech #smart #future #innovation',
    music: 'Deep Tech - AI Composer',
    likes: '256k',
    comments: '8.9k',
    shares: '12k'
  },
  {
    ...
    author: { name: 'mountain_man', avatar: 'https://i.pravatar.cc/150?u=4', followed: false },
    description: 'Above the clouds. Perspective is everything. ☁️⛰️ #mountains #travel #adventure',
    music: 'Cloud Walker - Ambient Vibes',
    likes: '45k',
    comments: '890',
    shares: '2.1k'
  }
...
const ReelItem: React.FC<{ reel: Reel; isActive: boolean; isMuted: boolean; toggleMute: () => void }> = ({ reel, isActive, isMuted, toggleMute }) => {
  ...
  const handleShare = async (e: React.MouseEvent) => {
    e.stopPropagation();
    const shareData = {
      title: `Sun Smart Lessons via @${reel.author.name}`,
      ...
      <AnimatePresence>
        {showCopied && (
          <motion.div 
            ...
            className="absolute top-20 left-1/2 -translate-x-1/2 z-[100] px-6 py-3 bg-sun-primary text-black font-black uppercase text-[10px] tracking-widest rounded-full shadow-2xl shadow-sun-primary/20 pointer-events-none"
          >
            Post Link Copied
          </motion.div>
        )}
            ...
            <AnimatePresence>
              {!isFollowed && (
                  <motion.div 
                    ...
                    className="absolute -bottom-1 -right-1 bg-sun-primary text-black rounded-full p-1 border-2 border-sun-bg"
                  >
    ...
    <div className="fixed inset-0 bg-sun-bg flex justify-center overflow-hidden z-0">
      ...
      <div className="absolute right-8 top-1/2 -translate-y-1/2 z-[50] hidden xl:flex flex-col gap-6">
        <button 
          onClick={() => scroll('up')}
          title="Previous Reel"
          className="w-12 h-12 rounded-full bg-sun-surface-light/80 backdrop-blur-xl border border-sun-border flex items-center justify-center text-sun-text-main hover:bg-sun-primary hover:text-black hover:scale-110 transition-all shadow-2xl"
